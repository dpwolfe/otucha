...
**--jsUseDist** - No longer supported. A form of this will return to re-enable git deploy.  Uses the otucha.js and index.html from the dist directory instead of building them with Emscripten.  Used for git deploy on a server that does not have CMake and Emscripten available.
